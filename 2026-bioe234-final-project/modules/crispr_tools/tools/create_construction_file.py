...
class CreateConstructionFile:
    ...
    Assembly strategies supported by Create_Construction_File ONLY:
    - GoldenGate
    - Gibson
    - TypeIISOligoCloning

    END OF CREATE_CONSTRUCTION_FILE ASSEMBLY STRATEGIES.
    The following discovered tools are separate MCP tools, not assembly strategies.

  Modes:
  1) Manual primer mode:
     Requires insert/vector primer names and sequences.

  2) Auto primer design mode (preferred):
     If auto_design_primers=True:
       ...
       - Only require:
           ...
           insertion_index
       - Optional:
           overlap_bp (default 20)
           primer_anneal_bp (default 20)

     The tool will automatically generate all primers.
    """
    ...
    def run(
        ...
        # -------- Gibson auto-primer design fields --------
        auto_design_primers: bool = False,
        insertion_index: int | None = None,
        overlap_bp: int = 20,
        primer_anneal_bp: int = 20,
        ...
        if assembly_strategy == "Gibson" and auto_design_primers:
            primer_design = self._design_gibson_primers(
                ...
                primer_anneal_bp=primer_anneal_bp,
                insert_name=insert_name,
                backbone_name=backbone_name,
            )
            insert_forward_primer_name = insert_forward_primer_name or primer_design["insert_forward_primer_name"]
            insert_forward_primer_sequence = insert_forward_primer_sequence or primer_design["insert_forward_primer_sequence"]
            insert_reverse_primer_name = insert_reverse_primer_name or primer_design["insert_reverse_primer_name"]
            insert_reverse_primer_sequence = insert_reverse_primer_sequence or primer_design["insert_reverse_primer_sequence"]
            vector_forward_primer_name = vector_forward_primer_name or primer_design["vector_forward_primer_name"]
            vector_forward_primer_sequence = vector_forward_primer_sequence or primer_design["vector_forward_primer_sequence"]
            vector_reverse_primer_name = vector_reverse_primer_name or primer_design["vector_reverse_primer_name"]
            vector_reverse_primer_sequence = vector_reverse_primer_sequence or primer_design["vector_reverse_primer_sequence"]

            gibson_note = (
                f"Auto-designed Gibson primers using insertion_index={insertion_index}, "
                f"overlap_bp={overlap_bp}, primer_anneal_bp={primer_anneal_bp}."
            )
            notes = f"{notes} {gibson_note}".strip()
        ...
        self._validate_user_inputs(
            ...
            enzyme=enzyme,
            auto_design_primers=auto_design_primers,
            insertion_index=insertion_index,
        )
    ...
    def _circular_slice(self, seq: str, start: int, end: int) -> str:
        """Return seq[start:end], allowing wraparound for circular plasmids."""
        seq = self._normalize_sequence(seq)
        if end < start:
            raise ValueError("end must be >= start for _circular_slice.")
        length = end - start
        if length > len(seq):
            raise ValueError("Cannot take a circular slice longer than the sequence.")
        return "".join(seq[i % len(seq)] for i in range(start, end))

    def _design_gibson_primers(
        self,
        backbone_sequence: str,
        insert_sequence: str,
        ...
        """Design four PCR primers for circular Gibson insertion."""
        backbone = self._normalize_sequence(backbone_sequence)
        insert = self._normalize_sequence(insert_sequence)

        if insertion_index is None:
            raise ValueError("insertion_index is required when auto_design_primers=True for Gibson.")
        if not isinstance(insertion_index, int):
            raise ValueError("insertion_index must be an integer.")
        if insertion_index < 0 or insertion_index > len(backbone):
            raise ValueError(f"insertion_index must be between 0 and {len(backbone)} for this backbone.")
        if not isinstance(overlap_bp, int) or overlap_bp < 15:
            raise ValueError("overlap_bp must be an integer >= 15 for Gibson assembly.")
        if not isinstance(primer_anneal_bp, int) or primer_anneal_bp < 12:
            raise ValueError("primer_anneal_bp must be an integer >= 12.")
        if len(backbone) < max(overlap_bp, primer_anneal_bp) * 2:
            raise ValueError("backbone_sequence is too short for Gibson primer design.")
        if len(insert) < primer_anneal_bp:
            raise ValueError("insert_sequence is too short for the requested primer_anneal_bp.")

        left_overlap = self._circular_slice(backbone, insertion_index - overlap_bp, insertion_index)
        right_overlap = self._circular_slice(backbone, insertion_index, insertion_index + overlap_bp)

        insert_f_anneal = insert[:primer_anneal_bp]
        insert_r_anneal = self._reverse_complement(insert[-primer_anneal_bp:])
        vector_f_anneal = self._circular_slice(backbone, insertion_index, insertion_index + primer_anneal_bp)
        vector_r_template = self._circular_slice(backbone, insertion_index - primer_anneal_bp, insertion_index)
        vector_r_anneal = self._reverse_complement(vector_r_template)

        safe_insert = self._sanitize_name(insert_name, "insert")
        safe_backbone = self._sanitize_name(backbone_name, "vector")

        return {
            "left_overlap": left_overlap,
            "right_overlap": right_overlap,
            "insert_forward_primer_name": f"{safe_insert}_Gib_F",
            "insert_forward_primer_sequence": left_overlap + insert_f_anneal,
            "insert_reverse_primer_name": f"{safe_insert}_Gib_R",
            "insert_reverse_primer_sequence": self._reverse_complement(right_overlap) + insert_r_anneal,
            "vector_forward_primer_name": f"{safe_backbone}_Gib_F",
            "vector_forward_primer_sequence": right_overlap + vector_f_anneal,
            "vector_reverse_primer_name": f"{safe_backbone}_Gib_R",
            "vector_reverse_primer_sequence": self._reverse_complement(left_overlap) + vector_r_anneal,
        }
